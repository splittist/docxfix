...
## Experiment Results (exp4–exp7, 2026-02-21)

After fixing a content-type typo (`openxmlformats.officedocument` → `openxmlformats-officedocument`) that caused Word to reject commentsIds.xml in the first generation:

| File | Config | Result |
|------|--------|--------|
| `exp4-case2-isolation.docx` | Unique docId + commentsIds, ONLY Case 2 | ❌ ALL separate |
| `exp5-cases-swapped.docx` | Unique docId + commentsIds, Cases 2&3 swapped | ❌ ALL separate |
| `exp6-unique-durable-ids.docx` | Unique docId + commentsIds, unique durableIds | ❌ ALL separate |
| `exp7-unique-docid-no-commentsids.docx` | Unique docId, NO commentsIds | ❌ ALL separate |

exp4/5/6 were generated by a minimal custom script (missing webSettings.xml, footnotes.xml, endnotes.xml, fontTable.xml, theme, docProps). exp7 also used the minimal script but opened without errors. All comments separate.

---

## Experiments Generated (2026-02-21, third batch): Full docxfix structure

exp4/5/6 used a minimal ZIP structure. exp3 (which partially worked) was generated by the full docxfix generator (15 ZIP entries). Three new experiments used the **actual docxfix generator** as a base, then post-processed to inject unique docId and/or commentsIds:

| File | Config | Notes |
|------|--------|-------|
| `exp8-docxfix-unique-docid-with-ids.docx` | Full docxfix + unique docId + commentsIds | Same config as exp3 |
| `exp9-docxfix-unique-docid-no-ids.docx` | Full docxfix + unique docId, NO commentsIds | Like exp1 but full structure |
| `exp10-docxfix-shared-docid-with-ids.docx` | Full docxfix + shared docId + commentsIds | Tests commentsIds without unique docId |

## Experiment Results (exp8–exp10, 2026-02-25)

| File | Result |
|------|--------|
| `exp8-docxfix-unique-docid-with-ids.docx` | ❌ ALL separate |
| `exp9-docxfix-unique-docid-no-ids.docx` | ❌ ALL separate |
| `exp10-docxfix-shared-docid-with-ids.docx` | ❌ ALL separate |

**Critical check (2026-02-25):** `debug-comments-simple-v4.docx` ✅ **STILL WORKS.**

---

## Final Analysis and Conclusions

### The Most Likely Explanation: Word Caches Threading State Per docId

All evidence points to Word maintaining a **persistent local cache of comment threading state, keyed by `w14:docId`**.

- v4 (docId `4D55359B`) was among the first files with this docId opened in Word. Word processed its `commentsExtended.xml` correctly and cached the threading state.
- v5, v6, and all exp files with docId `4D55359B` fail — even with identical structure — because Word looks up its cached state for that docId, finds a mismatch with the new file's paraIds, and falls back to flat display.
- Files with fresh unique docIds also fail, because Word appears to **only** use the cache for threading and does not re-process `commentsExtended.xml` live for fresh docIds. (The one exception was exp3 on 2026-02-21, which partially worked — this may have been a transient state before the cache was "full".)
- v4 still works on 2026-02-25 because its threading state is preserved in the cache.

### Why This Is a Dead End for Now

We cannot reproduce v4's success reliably because:
1. The cache is inside Word's application data (likely `%APPDATA%\Microsoft\Office\`) and is not programmatically accessible.
2. The OOXML spec (`commentsExtended.xml` threading via `w15:paraIdParent`) appears to be correctly implemented, but Word does not evaluate it for untrusted/programmatically-generated files.
3. Additional structures (`commentsIds.xml`, unique `w14:docId`) are necessary but not sufficient without the corresponding cache entry.

### What We Do Know Works

Our `commentsExtended.xml` generation is structurally correct (verified against Word-generated reference files and confirmed working in SuperDoc). The issue is Word-specific and likely requires either:
- A Word-managed file identity mechanism we cannot reproduce externally (e.g., the file being saved by Word itself, which populates additional state)
- Or access to whatever Microsoft's first-party tooling does to establish the comment cache

### Recommendations for the Library

1. **Keep `commentsExtended.xml`** with correct `w15:paraIdParent` threading — it is the correct per-spec approach and works in other OOXML renderers (SuperDoc, etc.)
2. **Add unique `w14:docId` per document** (currently hardcoded as `4D55359B`) — low-risk, required by the spec, and may help in some Word environments
3. **Add `commentsIds.xml`** — the code already exists; hook it up in the generator — it is present in all Word-generated threaded documents and appears necessary
4. **Document the limitation** — generated `.docx` files with threaded comments render correctly in OOXML-compliant renderers; Microsoft Word's threading display depends on internal application state that cannot be fully controlled programmatically
